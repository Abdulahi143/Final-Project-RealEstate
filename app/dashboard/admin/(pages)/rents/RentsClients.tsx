...
const location = getByValue(listings.find((listing) => listing.locationValue)?.locationValue || '');
  ...
  const onDelete = useCallback(
    ...
    listing.type === "RENT" && // Filter only sale listings
...
<div className="relative mt-24">
  <div className="relative">
    <input
      type="text"
      placeholder="Search..."
      className="px-10 py-2 border rounded focus:outline-none focus:ring focus:border-blue-300 dark:bg-gray-800 dark:border-gray-700 dark:focus:border-blue-500 dark:focus:ring-blue-500"
      value={searchTerm}
    ...
    <FaSearch className="absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-500" />
      ...
      <div className="flex items-center justify-center space-x-4">
          ...
          <div
            className={`cursor-pointer px-4 py-2 bg-green-500 text-white rounded-md ${
              activeTab === "rented" ? "opacity-100" : "opacity-70 hover:opacity-100"
            }`}
          ...
          <tbody>
            {currentListings.map((listing: any) => (
              ...
              >
                ...
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                {findUserById(listing.userId)?.name || 'Unknown user'}
                </td>
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  {location?.label}
                </td>
